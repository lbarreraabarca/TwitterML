...
public class main
{
	public static void main( String[] args ) throws FileNotFoundException, IOException, InterruptedException
  {
    ...
    else
    {
			...
			System.out.print( "Cargando Bolsas de Palabras, Hashtags, User Mentions " );
      ArrayList < String >  palabras_Busqueda  = cargarBW( args[ 1 ] ); /* Palabra de busqueda. KeyWords por ejemplo Terremoto*/
			ArrayList < String >  bolsaPalabrasHashtags = cargarBW( args[ 6 ] ) ; /*Bolsa de hashtags. */
			ArrayList < String >  bolsaUserMentions = cargarBW( args[ 7 ] ) ; /* Bolsa de USERMENTIONS*/
			ArrayList < String >  bolsaPalabras = cargarBW( args[ 5 ] ) ; /* Bolsa de Palabras */
      ...
      try
      {
				...
				while( ( linea = br2.readLine( ) ) != null)
        {
         	...
         	String etiqueta = data[ 10 ]; /* Etiqueta del tuit puede ser PRECRISIS, CRISIS, PRELUDIO. */
					...
					//pwVEST.println( estructurales + etiqueta );
          //pwVBOW.println( temporales + etiqueta );
          //pwVBOH.println( hashTagsCaracteristicas + etiqueta );
          //pwVBUM.println( userMentionsCaracteristicas + etiqueta );
				}
				br2.close( );
    ...
    while( st.hasMoreTokens( ) )
    {
      String word = st.nextToken( ); /* palabra */
		...
		String vectorSalida = "";
		for( int i = 0; i < bolsaPalabras.size( ); i++ )
      //vectorSalida = vectorSalida + vectorTemporalidad[ i ]  + ",";
			/* SVMLight Format */
